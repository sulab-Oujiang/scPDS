...
PARAMETERS = {
    "Afatinib": {"T_sample": "SMOTEsampling", "T_batch_size": 150, "T_embedding_dim": 1300, 
                 "P_dropout": 0.4, "P_weight_decay": 0, "P_input_dim": 256, "P_hidden_dim": [64,32]},
    "Cetuximab": {"T_sample": "DOWNsampling", "T_batch_size": 200, "T_embedding_dim": 512, 
                 "P_dropout": 0.5, "P_weight_decay": 0.01, "P_input_dim": 256, "P_hidden_dim": [64,32]},
    "NVP-TAE684": {"T_sample": "DOWNsampling", "T_batch_size": 150, "T_embedding_dim": 1300, 
                 "P_dropout": 0.5, "P_weight_decay": 0, "P_input_dim": 256, "P_hidden_dim": [128,64]},
    "Sorafenib": {"T_sample": "NOsampling", "T_batch_size": 150, "T_embedding_dim": 512, 
                 "P_dropout": 0.4, "P_weight_decay": 0, "P_input_dim": 256, "P_hidden_dim": [64,32]},
    "AR-42": {"T_sample": "UPsampling", "T_batch_size": 150, "T_embedding_dim": 512, 
                 "P_dropout": 0.2, "P_weight_decay": 0, "P_input_dim": 256, "P_hidden_dim": [64,32]},
    "Gefitinib": {"T_sample": "DOWNsampling", "T_batch_size": 150, "T_embedding_dim": 512, 
                 "P_dropout": 0.5, "P_weight_decay": 0, "P_input_dim": 256, "P_hidden_dim": [64,32]},
    "Vorinostat": {"T_sample": "UPsampling", "T_batch_size": 150, "T_embedding_dim": 512, 
                 "P_dropout": 0.5, "P_weight_decay": 0, "P_input_dim": 128, "P_hidden_dim": [64,32]},
    "PLX4720_451Lu": {"T_sample": "NOsampling", "T_batch_size": 150, "T_embedding_dim": 512, 
                 "P_dropout": 0.2, "P_weight_decay": 0.1, "P_input_dim": 256, "P_hidden_dim": [64,32]},
    "PLX4720_A375": {"T_sample": "SMOTEsampling", "T_batch_size": 150, "T_embedding_dim": 512, 
                 "P_dropout": 0.3, "P_weight_decay": 0, "P_input_dim": 256, "P_hidden_dim": [128,64]},
    "Etoposide": {"T_sample": "SMOTEsampling", "T_batch_size": 200, "T_embedding_dim": 1288, 
                 "P_dropout": 0.4, "P_weight_decay": 0.09, "P_input_dim": 128, "P_hidden_dim": [64,32]},
}
...
##############################################
Drug = "Afatinib"
params = PARAMETERS.get(Drug)

# File Path
Bulk_label_path = "Data/Sc_Benchmark/" + Drug + "/Source_label.csv"
Bulk_data_path = "Data/Sc_Benchmark/" + Drug + "/Source_Pathway_Active_Score.csv"
Sc_label_path = "Data/Sc_Benchmark/" + Drug + "/Target_label.csv"
Sc_data_path = "Data/Sc_Benchmark/" + Drug + "/Target_Pathway_Active_Score.csv"

# Transformer
T_sample = params["T_sample"]                  # sampling strategy: NOsampling, DOWNsampling, UPsampling, SMOTEsampling.
T_batch_size = params["T_batch_size"]          # the number of data samples included in each batch
T_lr = 1e-4                                    # learning rate of Transformer
T_dropout = 0                                  # dropout
T_weight_decay = 0                             # weight decay
T_embedding_dim = params["T_embedding_dim"]    # embedded dimension
T_Bottleneck_dim = 256                         # extracted dimension
...
P_dropout = params["P_dropout"]                # dropout of Predictor
P_weight_decay = params["P_weight_decay"]      # weight decay of Predictor
P_input_dim = params["P_input_dim"]            # input dimension of Predictor
P_hidden_dim = params["P_hidden_dim"]          # hidden dimension of Predictor
...
Bulk_label = pd.read_csv(Bulk_label_path, index_col=0)

# Read Sc data
Sc_data = pd.read_csv(Sc_data_path, index_col=0)
Sc_label = pd.read_csv(Sc_label_path, index_col=0)
...
Y = Sc_label.values.reshape(Sc_label.shape[0],)

# The confidence level of the last column（Optimal Threshold）
fpr, tpr, thresholds = roc_curve(Y, sensitive_score)
optimal_idx = np.argmax(tpr - fpr)         
optimal_threshold = thresholds[optimal_idx]
y_pred = (sensitive_score > optimal_threshold).astype(int)

# Save
pd.DataFrame(sensitive_score).to_csv('Result/Sc_Result/sensitive_score.csv')
pd.DataFrame(y_pred).to_csv("Result/Sc_Result/Predict.csv")

# Evaluate
AUC = roc_auc_score(np.array(Y), np.array(sensitive_score))
AP = average_precision_score(np.array(Y), np.array(sensitive_score))
Accuracy = accuracy_score(Y, y_pred)
report_dict = pd.DataFrame(classification_report(Y, y_pred, output_dict=True))
Precision = report_dict["weighted avg"]["precision"]
Recall = report_dict["weighted avg"]["recall"]
f1 = report_dict["weighted avg"]["f1-score"]
print("Evaluate SC Data")
print("AUC = ", round(AUC, 3))
print("AP = ", round(AP, 3))
print("Accuracy = ", round(Accuracy, 3))
print("Precision = ", round(Precision, 3))
print("Recall = ", round(Recall, 3))
print("f1 = ", round(f1, 3))
logging.info("Evaluate Sc Data")
logging.info("AUC = %s", round(AUC, 3))
logging.info("AP = %s", round(AP, 3))
logging.info("Accuracy = %s", round(Accuracy, 3))
logging.info("Precision = %s", round(Precision, 3))
logging.info("Recall = %s", round(Recall, 3))
logging.info("f1 = %s", round(f1, 3))
